define(function (require, exports, module) {
    ...
    FindInFiles.doSearchInScope(queryInfo, scope).then(function (results) {
      ...
      for (var path in results) {
        ...
        if (results.hasOwnProperty(path)) {
          ...
          var relativePath = FileUtils.getRelativeFilename(basePath, path).replace("/" + filename,"");
      ...
      $('#filter-text').fastLiveFilter('#snippets', {
        ...
        callback: function (total) {
          $('#num-results').html(total + " documents");
        },
        selector: ".path"
      });
  ...
  function results(regex) {
    ...
    FindInFiles.doSearchInScope(queryInfo, scope).then(function (results) {
      ...
      for (var path in results) {
        ...
        if (results.hasOwnProperty(path) && filename.charAt(0) != "." && typeof relativePath != 'undefined') {
          output += "<li><div class='filename'>" + filename + "</div>";
          output += "<div class='path'>" + relativePath + "</div>";
          var matches = results[path]["matches"];
          for (var i = 0; i < matches.length; i++) {
  ...
  function hashtags() {
    ...
    var tagCounts = {};
    //  FindInFiles.doSearchInScope(queryInfo, scope).then(function (results) {
    FindInFiles.doSearchInScope(queryInfo).then(function () {
      FindInFiles.getAllSearchResults().then(function () {
        var results = FindInFiles.searchModel.results;
